...
public final class QueryUtils {
    ...
    //query the USGS dataset and return the {earthquake } object to represent a single earthquake
    public static ArrayList<Earthquakes> fetchEarthquakeData(String requestURL){
        ...
        //return the array of {earthquakes} object
        return earthquake;
    }

    /*
        creates new url object from the given string url
    */
    ...
    private static String makeHttpRequest(URL url) throws IOException{
        ...
        try{
            ...
            //if the request was successful then we will get response code 200
    ...
    * */
    ...
    //return a list of objects that has been built from parsing a json response
    private static ArrayList<Earthquakes> extractFeatureFromJson(String jsonResponse){

        //create an empty earthquake list that we can start adding earthquakes to
        ArrayList<Earthquakes> earthquakes=new ArrayList<>();
        //handles the case when the json response is null , because we entered wrong url
        ...
        try {
            ...
            //number of earthquake data objects in the array
            int noOfObjects=features.length();
            //looping through all the data objects
            for(int i=0;i<noOfObjects;i++){
                JSONObject c=features.getJSONObject(i);
                JSONObject properties=c.getJSONObject("properties");
                String magnitude=""+properties.optDouble("mag");
                String place=properties.getString("place");
                long time=properties.getLong("time");
                Date dateObject=new Date(time);
                //thus we need a date formater to convert this date format into String
                String datetoDisplay=dateFormat(dateObject);
                //format the time string
                String timeToDisplay=timeFormat(dateObject);
                String pageUrl=properties.getString("url");
                earthquakes.add(new Earthquakes(magnitude,place,datetoDisplay,timeToDisplay,pageUrl));
            }
    ...
    private static String dateFormat(Date dateObject) {
        SimpleDateFormat dateFormatter=new SimpleDateFormat("MMM dd, yyyy");
        return dateFormatter.format(dateObject);
    }
    private static String timeFormat(Date dateObject){
